<%-- 
    Document   : nav
    Created on : Dec 17, 2013, 9:00:14 PM
    Author     : CHUNG TOOC
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<ul class="nav">
    <li><a href="./welcome.jsp" class="nav-home"></a></li>
    <li><a href="./welcome.jsp" class="nav-bookings focus"></a></li>
    <li><a href="./welcome.jsp" class="nav-rooms"></a></li>					
    <li><a href="./welcome.jsp" class="nav-reports"></a></li>
    <li><a href="./welcome.jsp" class="nav-invoices"></a></li>
    <li><a href="./logout.jsp" class="nav-logout"></a></li>
</ul>
<ul class="subnav">
    <li><a href="./welcome.jsp" class="">Find</a></li>
    <li><a href="./welcome.jsp" class="">Add Booking</a></li>
    <li><a href="./welcome.jsp" class="focus">All Bookings</a></li>
    <li><a href="./welcome.jsp" class="">Calendar</a></li>
</ul>
